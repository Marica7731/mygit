(function () {
  ...
  ready(() => {
    ...
    fetch("data/youtube-ranking.json")
      ...
      .catch(() => {});
    refresh();
    for (let i = 1; i <= 10; i += 1) setTimeout(refresh, i * 450);
  ...
  function refresh() {
    ...
    document.querySelectorAll(".video-card").forEach((card) => {
      ...
      card.querySelectorAll(".hb-metric,.hb-meta,.hb-thumb-chip,.rank-line .rank-metric,.original-rank,.meta-list,.id-line,.status-pill.video").forEach((node) => node.remove());
      if (!item) return;
      addMetric(card, item);
      addMeta(card, item);
      addThumbChips(card, item);
  ...
  function addThumbChips(card, item) {
    const thumb = card.querySelector(".thumbnail");
    if (!thumb) return;
    const chips = [
      ["keyword", item.keyword || item.group],
      ["duration", duration(item.durationText)],
    ].filter(([, value]) => value);
